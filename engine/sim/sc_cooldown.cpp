...
void cooldown_t::reset( bool require_reaction, int charges_ )
{
  ...
  this->reset_core( require_reaction );
}

void cooldown_t::reset_core( bool require_reaction )
{
  bool was_down = down();
  ready         = ready_init();
...
std::unique_ptr<expr_t> cooldown_t::create_expression( util::string_view name_str )
{
  ...
  {
    return make_fn_expr( name_str, [this] { return cooldown_duration( this ); } );
  }
